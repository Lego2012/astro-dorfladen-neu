jQuery(document).ready(function ($) {
		...
		$('#home-slider-canvas').flexslider({
			...
			after: function () {
				if (!$('.home-banner > i.flex-active-slide > a > img').length) {
					$('.home-banner')
						.find('li.flex-active-slide > img')
						.wrap("<a class='fancybox fancybox_listing_link' href='" + $('li.flex-active-slide img').data('full-image') + "'></a>");
					$('.fancybox').fancybox();
				}
			},
			start: function () {
	...
	function print_tabs() {
		if (!$('.print_tabs').length) {
			...
			var google_map =
				...
				'&zoom=' +
				zoom +
				'&size=700x200&markers=color:blue|label:S|' +
				latitude +
				',' +
				longitude +
				"&sensor=false'>";
			...
			$('.example-tabs').each(function () {
				...
				$(this)
					...
					.each(function (index, element) {
						...
						tabs_html +=
							($(this).find('a').attr('href') == '#location' ? google_map : $('.tab-content .tab-pane').eq(index).html()) + '</div><br /><br />';
					});
	...
	function print_images() {
		if (!$('.print_image').length) {
			...
			$('.print_tabs').prepend(
				"<div class='print_friendly print_image'>" +
					images_html +
					"<br></div><div class='car-info'>" +
					car_info +
